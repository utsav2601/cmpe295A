 ...
 */
static  errval_t monitor_mem_alloc__rpc(struct monitor_mem_rpc_client *_rpc, uint8_t size_bits, monitor_mem_genpaddr_t minbase, monitor_mem_genpaddr_t maxlimit, monitor_mem_coreid_t from, monitor_mem_errval_t *err, monitor_mem_caprep_t *caprep)
{
    ...
    // call send function
    _err = ((((_rpc->b)->tx_vtbl).alloc_call)(_rpc->b, NOP_CONT, size_bits, minbase, maxlimit, from));
    if (err_is_fail(_err)) {
        goto out;
    }
...
static  errval_t monitor_mem_free__rpc(struct monitor_mem_rpc_client *_rpc, monitor_mem_caprep_t caprep, monitor_mem_genpaddr_t base, uint8_t size_bits, monitor_mem_errval_t *status)
{
    errval_t _err = SYS_ERR_OK;
    assert(!(_rpc->rpc_in_progress));
    assert((_rpc->async_error) == SYS_ERR_OK);
    ...
    // call send function
    _err = ((((_rpc->b)->tx_vtbl).free_call)(_rpc->b, NOP_CONT, caprep, base, size_bits));
    if (err_is_fail(_err)) {
        goto out;
    }
    ...
    // grab reply variables out of binding
    struct monitor_mem_binding *_binding = _rpc->b;
    *status = (((_binding->rx_union).free_response).status);
...
static  void monitor_mem_free__rpc_rx_handler(struct monitor_mem_binding *_binding, monitor_mem_errval_t status)
{
    // get RPC client state pointer
    struct monitor_mem_rpc_client *_rpc = _binding->st;
    ...
    // XXX: stash reply parameters in binding object
    // depending on the interconnect driver, they're probably already there
    ((_binding->rx_union).free_response).status = status;
 ...
 errval_t monitor_mem_rpc_client_init(struct monitor_mem_rpc_client *rpc, struct monitor_mem_binding *binding)
{
    ...
    (binding->rx_vtbl).free_response = monitor_mem_free__rpc_rx_handler;
