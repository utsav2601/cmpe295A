...
static  errval_t acpi_mm_realloc_range_proxy__rpc(struct acpi_rpc_client *_rpc, uint8_t sizebits, uint64_t minbase, struct capref *devframe, acpi_errval_t *err)
{
    errval_t _err = SYS_ERR_OK;
    assert(!(_rpc->rpc_in_progress));
    assert((_rpc->async_error) == SYS_ERR_OK);
    ...
    // call send function
    _err = ((((_rpc->b)->tx_vtbl).mm_realloc_range_proxy_call)(_rpc->b, NOP_CONT, sizebits, minbase));
    if (err_is_fail(_err)) {
        goto out;
    }
    ...
    // grab reply variables out of binding
    struct acpi_binding *_binding = _rpc->b;
    *devframe = (((_binding->rx_union).mm_realloc_range_proxy_response).devframe);
    *err = (((_binding->rx_union).mm_realloc_range_proxy_response).err);
...
static  void acpi_mm_realloc_range_proxy__rpc_rx_handler(struct acpi_binding *_binding, struct capref devframe, acpi_errval_t err)
{
    // get RPC client state pointer
    struct acpi_rpc_client *_rpc = _binding->st;
    ...
    // XXX: stash reply parameters in binding object
    // depending on the interconnect driver, they're probably already there
    ((_binding->rx_union).mm_realloc_range_proxy_response).devframe = devframe;
    ((_binding->rx_union).mm_realloc_range_proxy_response).err = err;
...
static  struct acpi_rpc_vtbl acpi_rpc_vtbl = {
    ...
    .mm_realloc_range_proxy = acpi_mm_realloc_range_proxy__rpc,
