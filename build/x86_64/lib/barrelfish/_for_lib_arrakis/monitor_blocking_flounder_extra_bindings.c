...
static  void monitor_blocking_get_ipi_cap_call__lmp_send_handler(void *arg)
{
    // Get the binding state from our argument pointer
    struct monitor_blocking_binding *_binding = arg;
    struct monitor_blocking_lmp_binding *b = arg;
    ...
    switch (_binding->tx_msg_fragment) {
    case 0:
        err = lmp_chan_send1(&(b->chan), b->flags, NULL_CAP, monitor_blocking_get_ipi_cap_call__msgnum);
        if (err_is_ok(err)) {
            _binding->tx_msgnum = 0;
            flounder_support_trigger_chan(&(_binding->tx_cont_chanstate));
            flounder_support_trigger_chan(&(_binding->register_chanstate));
    ...
    if (lmp_err_is_transient(err)) {
        // Construct retry closure and register it
        struct event_closure retry_closure = (struct event_closure){  .handler = monitor_blocking_get_ipi_cap_call__lmp_send_handler,  .arg = arg };
        err = lmp_chan_register_send(&(b->chan), _binding->waitset, retry_closure);
        assert(err_is_ok(err));
    } else {
        // Report error to user
...
static  void monitor_blocking_get_ipi_cap_response__lmp_send_handler(void *arg)
{
    // Get the binding state from our argument pointer
    struct monitor_blocking_binding *_binding = arg;
    struct monitor_blocking_lmp_binding *b = arg;
    ...
    switch (_binding->tx_msg_fragment) {
    case 0:
        err = lmp_chan_send1(&(b->chan), b->flags, ((_binding->tx_union).get_ipi_cap_response).cap, monitor_blocking_get_ipi_cap_response__msgnum);
        if (err_is_ok(err)) {
            _binding->tx_msgnum = 0;
            flounder_support_trigger_chan(&(_binding->tx_cont_chanstate));
            flounder_support_trigger_chan(&(_binding->register_chanstate));
    ...
    if (lmp_err_is_transient(err)) {
        // Construct retry closure and register it
        struct event_closure retry_closure = (struct event_closure){  .handler = monitor_blocking_get_ipi_cap_response__lmp_send_handler,  .arg = arg };
        err = lmp_chan_register_send(&(b->chan), _binding->waitset, retry_closure);
        assert(err_is_ok(err));
    } else {
        // Report error to user
...
static  void monitor_blocking_forward_kcb_request_call__lmp_send_handler(void *arg)
{
    // Get the binding state from our argument pointer
    struct monitor_blocking_binding *_binding = arg;
    struct monitor_blocking_lmp_binding *b = arg;
    ...
    switch (_binding->tx_msg_fragment) {
    case 0:
        err = lmp_chan_send1(&(b->chan), b->flags, ((_binding->tx_union).forward_kcb_request_call).kcb, monitor_blocking_forward_kcb_request_call__msgnum | (((uintptr_t )(((_binding->tx_union).forward_kcb_request_call).destination)) << 16));
        if (err_is_ok(err)) {
            _binding->tx_msgnum = 0;
            flounder_support_trigger_chan(&(_binding->tx_cont_chanstate));
            flounder_support_trigger_chan(&(_binding->register_chanstate));
    ...
    if (lmp_err_is_transient(err)) {
        // Construct retry closure and register it
        struct event_closure retry_closure = (struct event_closure){  .handler = monitor_blocking_forward_kcb_request_call__lmp_send_handler,  .arg = arg };
        err = lmp_chan_register_send(&(b->chan), _binding->waitset, retry_closure);
        assert(err_is_ok(err));
    } else {
        // Report error to user
...
static  void monitor_blocking_forward_kcb_request_response__lmp_send_handler(void *arg)
{
    // Get the binding state from our argument pointer
    struct monitor_blocking_binding *_binding = arg;
    struct monitor_blocking_lmp_binding *b = arg;
    ...
    switch (_binding->tx_msg_fragment) {
    case 0:
        err = lmp_chan_send2(&(b->chan), b->flags, NULL_CAP, monitor_blocking_forward_kcb_request_response__msgnum, ((_binding->tx_union).forward_kcb_request_response).err);
        if (err_is_ok(err)) {
            _binding->tx_msgnum = 0;
            flounder_support_trigger_chan(&(_binding->tx_cont_chanstate));
            flounder_support_trigger_chan(&(_binding->register_chanstate));
    ...
    if (lmp_err_is_transient(err)) {
        // Construct retry closure and register it
        struct event_closure retry_closure = (struct event_closure){  .handler = monitor_blocking_forward_kcb_request_response__lmp_send_handler,  .arg = arg };
        err = lmp_chan_register_send(&(b->chan), _binding->waitset, retry_closure);
        assert(err_is_ok(err));
    } else {
        // Report error to user
...
static  void monitor_blocking_forward_kcb_rm_request_call__lmp_send_handler(void *arg)
{
    // Get the binding state from our argument pointer
    struct monitor_blocking_binding *_binding = arg;
    struct monitor_blocking_lmp_binding *b = arg;
    ...
    switch (_binding->tx_msg_fragment) {
    case 0:
        err = lmp_chan_send1(&(b->chan), b->flags, ((_binding->tx_union).forward_kcb_rm_request_call).kcb, monitor_blocking_forward_kcb_rm_request_call__msgnum | (((uintptr_t )(((_binding->tx_union).forward_kcb_rm_request_call).destination)) << 16));
        if (err_is_ok(err)) {
            _binding->tx_msgnum = 0;
            flounder_support_trigger_chan(&(_binding->tx_cont_chanstate));
            flounder_support_trigger_chan(&(_binding->register_chanstate));
    ...
    if (lmp_err_is_transient(err)) {
        // Construct retry closure and register it
        struct event_closure retry_closure = (struct event_closure){  .handler = monitor_blocking_forward_kcb_rm_request_call__lmp_send_handler,  .arg = arg };
        err = lmp_chan_register_send(&(b->chan), _binding->waitset, retry_closure);
        assert(err_is_ok(err));
    } else {
        // Report error to user
...
static  void monitor_blocking_forward_kcb_rm_request_response__lmp_send_handler(void *arg)
{
    // Get the binding state from our argument pointer
    struct monitor_blocking_binding *_binding = arg;
    struct monitor_blocking_lmp_binding *b = arg;
    ...
    switch (_binding->tx_msg_fragment) {
    case 0:
        err = lmp_chan_send2(&(b->chan), b->flags, NULL_CAP, monitor_blocking_forward_kcb_rm_request_response__msgnum, ((_binding->tx_union).forward_kcb_rm_request_response).err);
        if (err_is_ok(err)) {
            _binding->tx_msgnum = 0;
            flounder_support_trigger_chan(&(_binding->tx_cont_chanstate));
            flounder_support_trigger_chan(&(_binding->register_chanstate));
    ...
    if (lmp_err_is_transient(err)) {
        // Construct retry closure and register it
        struct event_closure retry_closure = (struct event_closure){  .handler = monitor_blocking_forward_kcb_rm_request_response__lmp_send_handler,  .arg = arg };
        err = lmp_chan_register_send(&(b->chan), _binding->waitset, retry_closure);
        assert(err_is_ok(err));
    } else {
        // Report error to user
...
static  void monitor_blocking_get_global_paddr_call__lmp_send_handler(void *arg)
{
    // Get the binding state from our argument pointer
    struct monitor_blocking_binding *_binding = arg;
    struct monitor_blocking_lmp_binding *b = arg;
    ...
    switch (_binding->tx_msg_fragment) {
    case 0:
        err = lmp_chan_send1(&(b->chan), b->flags, NULL_CAP, monitor_blocking_get_global_paddr_call__msgnum);
        if (err_is_ok(err)) {
            _binding->tx_msgnum = 0;
            flounder_support_trigger_chan(&(_binding->tx_cont_chanstate));
            flounder_support_trigger_chan(&(_binding->register_chanstate));
    ...
    if (lmp_err_is_transient(err)) {
        // Construct retry closure and register it
        struct event_closure retry_closure = (struct event_closure){  .handler = monitor_blocking_get_global_paddr_call__lmp_send_handler,  .arg = arg };
        err = lmp_chan_register_send(&(b->chan), _binding->waitset, retry_closure);
        assert(err_is_ok(err));
    } else {
        // Report error to user
...
static  void monitor_blocking_get_global_paddr_response__lmp_send_handler(void *arg)
{
    // Get the binding state from our argument pointer
    struct monitor_blocking_binding *_binding = arg;
    struct monitor_blocking_lmp_binding *b = arg;
    ...
    switch (_binding->tx_msg_fragment) {
    case 0:
        err = lmp_chan_send2(&(b->chan), b->flags, NULL_CAP, monitor_blocking_get_global_paddr_response__msgnum, ((_binding->tx_union).get_global_paddr_response).global);
        if (err_is_ok(err)) {
            _binding->tx_msgnum = 0;
            flounder_support_trigger_chan(&(_binding->tx_cont_chanstate));
            flounder_support_trigger_chan(&(_binding->register_chanstate));
    ...
    if (lmp_err_is_transient(err)) {
        // Construct retry closure and register it
        struct event_closure retry_closure = (struct event_closure){  .handler = monitor_blocking_get_global_paddr_response__lmp_send_handler,  .arg = arg };
        err = lmp_chan_register_send(&(b->chan), _binding->waitset, retry_closure);
        assert(err_is_ok(err));
    } else {
        // Report error to user
...
static  errval_t monitor_blocking_get_ipi_cap_call__lmp_send(struct monitor_blocking_binding *_binding, struct event_closure _continuation)
{
    // check that we can accept an outgoing message
    if ((_binding->tx_msgnum) != 0) {
        return(FLOUNDER_ERR_TX_BUSY);
    ...
    // store message number and arguments
    _binding->tx_msgnum = monitor_blocking_get_ipi_cap_call__msgnum;
    _binding->tx_msg_fragment = 0;
    FL_DEBUG("lmp TX monitor_blocking.get_ipi_cap_call\n");
    
    // try to send!
    monitor_blocking_get_ipi_cap_call__lmp_send_handler(_binding);
    
    return(SYS_ERR_OK);
}

static  errval_t monitor_blocking_get_ipi_cap_response__lmp_send(struct monitor_blocking_binding *_binding, struct event_closure _continuation, struct capref cap)
{
    // check that we can accept an outgoing message
    if ((_binding->tx_msgnum) != 0) {
        return(FLOUNDER_ERR_TX_BUSY);
    ...
    // store message number and arguments
    _binding->tx_msgnum = monitor_blocking_get_ipi_cap_response__msgnum;
    _binding->tx_msg_fragment = 0;
    ((_binding->tx_union).get_ipi_cap_response).cap = cap;
    FL_DEBUG("lmp TX monitor_blocking.get_ipi_cap_response\n");
    
    // try to send!
    monitor_blocking_get_ipi_cap_response__lmp_send_handler(_binding);
    
    return(SYS_ERR_OK);
}

static  errval_t monitor_blocking_forward_kcb_request_call__lmp_send(struct monitor_blocking_binding *_binding, struct event_closure _continuation, monitor_blocking_coreid_t destination, struct capref kcb)
{
    // check that we can accept an outgoing message
    if ((_binding->tx_msgnum) != 0) {
        return(FLOUNDER_ERR_TX_BUSY);
    ...
    // store message number and arguments
    _binding->tx_msgnum = monitor_blocking_forward_kcb_request_call__msgnum;
    _binding->tx_msg_fragment = 0;
    ((_binding->tx_union).forward_kcb_request_call).destination = destination;
    ((_binding->tx_union).forward_kcb_request_call).kcb = kcb;
    FL_DEBUG("lmp TX monitor_blocking.forward_kcb_request_call\n");
    
    // try to send!
    monitor_blocking_forward_kcb_request_call__lmp_send_handler(_binding);
    
    return(SYS_ERR_OK);
}

static  errval_t monitor_blocking_forward_kcb_request_response__lmp_send(struct monitor_blocking_binding *_binding, struct event_closure _continuation, monitor_blocking_errval_t err)
{
    // check that we can accept an outgoing message
    if ((_binding->tx_msgnum) != 0) {
        return(FLOUNDER_ERR_TX_BUSY);
    ...
    // store message number and arguments
    _binding->tx_msgnum = monitor_blocking_forward_kcb_request_response__msgnum;
    _binding->tx_msg_fragment = 0;
    ((_binding->tx_union).forward_kcb_request_response).err = err;
    FL_DEBUG("lmp TX monitor_blocking.forward_kcb_request_response\n");
    
    // try to send!
    monitor_blocking_forward_kcb_request_response__lmp_send_handler(_binding);
    
    return(SYS_ERR_OK);
}

static  errval_t monitor_blocking_forward_kcb_rm_request_call__lmp_send(struct monitor_blocking_binding *_binding, struct event_closure _continuation, monitor_blocking_coreid_t destination, struct capref kcb)
{
    // check that we can accept an outgoing message
    if ((_binding->tx_msgnum) != 0) {
        return(FLOUNDER_ERR_TX_BUSY);
    ...
    // store message number and arguments
    _binding->tx_msgnum = monitor_blocking_forward_kcb_rm_request_call__msgnum;
    _binding->tx_msg_fragment = 0;
    ((_binding->tx_union).forward_kcb_rm_request_call).destination = destination;
    ((_binding->tx_union).forward_kcb_rm_request_call).kcb = kcb;
    FL_DEBUG("lmp TX monitor_blocking.forward_kcb_rm_request_call\n");
    
    // try to send!
    monitor_blocking_forward_kcb_rm_request_call__lmp_send_handler(_binding);
    
    return(SYS_ERR_OK);
}

static  errval_t monitor_blocking_forward_kcb_rm_request_response__lmp_send(struct monitor_blocking_binding *_binding, struct event_closure _continuation, monitor_blocking_errval_t err)
{
    // check that we can accept an outgoing message
    if ((_binding->tx_msgnum) != 0) {
        return(FLOUNDER_ERR_TX_BUSY);
    ...
    // store message number and arguments
    _binding->tx_msgnum = monitor_blocking_forward_kcb_rm_request_response__msgnum;
    _binding->tx_msg_fragment = 0;
    ((_binding->tx_union).forward_kcb_rm_request_response).err = err;
    FL_DEBUG("lmp TX monitor_blocking.forward_kcb_rm_request_response\n");
    
    // try to send!
    monitor_blocking_forward_kcb_rm_request_response__lmp_send_handler(_binding);
    
    return(SYS_ERR_OK);
}

static  errval_t monitor_blocking_get_global_paddr_call__lmp_send(struct monitor_blocking_binding *_binding, struct event_closure _continuation)
{
    // check that we can accept an outgoing message
    if ((_binding->tx_msgnum) != 0) {
        return(FLOUNDER_ERR_TX_BUSY);
    ...
    // store message number and arguments
    _binding->tx_msgnum = monitor_blocking_get_global_paddr_call__msgnum;
    _binding->tx_msg_fragment = 0;
    FL_DEBUG("lmp TX monitor_blocking.get_global_paddr_call\n");
    
    // try to send!
    monitor_blocking_get_global_paddr_call__lmp_send_handler(_binding);
    
    return(SYS_ERR_OK);
}

static  errval_t monitor_blocking_get_global_paddr_response__lmp_send(struct monitor_blocking_binding *_binding, struct event_closure _continuation, monitor_blocking_genpaddr_t global)
{
    // check that we can accept an outgoing message
    if ((_binding->tx_msgnum) != 0) {
        return(FLOUNDER_ERR_TX_BUSY);
    ...
    // store message number and arguments
    _binding->tx_msgnum = monitor_blocking_get_global_paddr_response__msgnum;
    _binding->tx_msg_fragment = 0;
    ((_binding->tx_union).get_global_paddr_response).global = global;
    FL_DEBUG("lmp TX monitor_blocking.get_global_paddr_response\n");
    
    // try to send!
    monitor_blocking_get_global_paddr_response__lmp_send_handler(_binding);
...
static  struct monitor_blocking_tx_vtbl monitor_blocking_lmp_tx_vtbl = {
    ...
    .get_ipi_cap_call = monitor_blocking_get_ipi_cap_call__lmp_send,
    .get_ipi_cap_response = monitor_blocking_get_ipi_cap_response__lmp_send,
    .forward_kcb_request_call = monitor_blocking_forward_kcb_request_call__lmp_send,
    .forward_kcb_request_response = monitor_blocking_forward_kcb_request_response__lmp_send,
    .forward_kcb_rm_request_call = monitor_blocking_forward_kcb_rm_request_call__lmp_send,
    .forward_kcb_rm_request_response = monitor_blocking_forward_kcb_rm_request_response__lmp_send,
    .get_global_paddr_call = monitor_blocking_get_global_paddr_call__lmp_send,
    .get_global_paddr_response = monitor_blocking_get_global_paddr_response__lmp_send,
};
/*
 * Receive handler
 */
 void monitor_blocking_lmp_rx_handler(void *arg)
{
    ...
    do {
        ...
        case monitor_blocking_get_arch_core_id_response__msgnum:
            ...
            break;
        case monitor_blocking_get_ipi_cap_call__msgnum:
            switch (_binding->rx_msg_fragment) {
            case 0:
                // check length
                if (((msg.buf).msglen) > 4) {
                ...
                FL_DEBUG("lmp RX monitor_blocking.get_ipi_cap_call\n");
                assert(((_binding->rx_vtbl).get_ipi_cap_call) != NULL);
                ((_binding->rx_vtbl).get_ipi_cap_call)(_binding);
                _binding->rx_msgnum = 0;
                break;
            default:
                (_binding->error_handler)(_binding, FLOUNDER_ERR_INVALID_STATE);
            ...
            break;
        case monitor_blocking_get_ipi_cap_response__msgnum:
            switch (_binding->rx_msg_fragment) {
            case 0:
                // check length
                if (((msg.buf).msglen) > 4) {
                ...
                ((_binding->rx_union).get_ipi_cap_response).cap = cap;
                
                FL_DEBUG("lmp RX monitor_blocking.get_ipi_cap_response\n");
                assert(((_binding->rx_vtbl).get_ipi_cap_response) != NULL);
                ((_binding->rx_vtbl).get_ipi_cap_response)(_binding, ((_binding->rx_union).get_ipi_cap_response).cap);
                _binding->rx_msgnum = 0;
                break;
            default:
                (_binding->error_handler)(_binding, FLOUNDER_ERR_INVALID_STATE);
            ...
            break;
        case monitor_blocking_forward_kcb_request_call__msgnum:
            switch (_binding->rx_msg_fragment) {
            case 0:
                // check length
                if (((msg.buf).msglen) > 4) {
                ...
                ((_binding->rx_union).forward_kcb_request_call).destination = ((((msg.words)[0]) >> 16) & 0xff);
                ((_binding->rx_union).forward_kcb_request_call).kcb = cap;
                
                FL_DEBUG("lmp RX monitor_blocking.forward_kcb_request_call\n");
                assert(((_binding->rx_vtbl).forward_kcb_request_call) != NULL);
                ((_binding->rx_vtbl).forward_kcb_request_call)(_binding, ((_binding->rx_union).forward_kcb_request_call).destination, ((_binding->rx_union).forward_kcb_request_call).kcb);
                _binding->rx_msgnum = 0;
                break;
            default:
                (_binding->error_handler)(_binding, FLOUNDER_ERR_INVALID_STATE);
            ...
            break;
        case monitor_blocking_forward_kcb_request_response__msgnum:
            switch (_binding->rx_msg_fragment) {
            case 0:
                // check length
                if (((msg.buf).msglen) > 4) {
                ...
                ((_binding->rx_union).forward_kcb_request_response).err = ((msg.words)[1]);
                
                FL_DEBUG("lmp RX monitor_blocking.forward_kcb_request_response\n");
                assert(((_binding->rx_vtbl).forward_kcb_request_response) != NULL);
                ((_binding->rx_vtbl).forward_kcb_request_response)(_binding, ((_binding->rx_union).forward_kcb_request_response).err);
                _binding->rx_msgnum = 0;
                break;
            default:
                (_binding->error_handler)(_binding, FLOUNDER_ERR_INVALID_STATE);
            ...
            break;
        case monitor_blocking_forward_kcb_rm_request_call__msgnum:
            switch (_binding->rx_msg_fragment) {
            case 0:
                // check length
                if (((msg.buf).msglen) > 4) {
                ...
                ((_binding->rx_union).forward_kcb_rm_request_call).destination = ((((msg.words)[0]) >> 16) & 0xff);
                ((_binding->rx_union).forward_kcb_rm_request_call).kcb = cap;
                
                FL_DEBUG("lmp RX monitor_blocking.forward_kcb_rm_request_call\n");
                assert(((_binding->rx_vtbl).forward_kcb_rm_request_call) != NULL);
                ((_binding->rx_vtbl).forward_kcb_rm_request_call)(_binding, ((_binding->rx_union).forward_kcb_rm_request_call).destination, ((_binding->rx_union).forward_kcb_rm_request_call).kcb);
                _binding->rx_msgnum = 0;
                break;
            default:
                (_binding->error_handler)(_binding, FLOUNDER_ERR_INVALID_STATE);
            ...
            break;
        case monitor_blocking_forward_kcb_rm_request_response__msgnum:
            switch (_binding->rx_msg_fragment) {
            case 0:
                // check length
                if (((msg.buf).msglen) > 4) {
                ...
                ((_binding->rx_union).forward_kcb_rm_request_response).err = ((msg.words)[1]);
                
                FL_DEBUG("lmp RX monitor_blocking.forward_kcb_rm_request_response\n");
                assert(((_binding->rx_vtbl).forward_kcb_rm_request_response) != NULL);
                ((_binding->rx_vtbl).forward_kcb_rm_request_response)(_binding, ((_binding->rx_union).forward_kcb_rm_request_response).err);
                _binding->rx_msgnum = 0;
                break;
            default:
                (_binding->error_handler)(_binding, FLOUNDER_ERR_INVALID_STATE);
            ...
            break;
        case monitor_blocking_get_global_paddr_call__msgnum:
            switch (_binding->rx_msg_fragment) {
            case 0:
                // check length
                if (((msg.buf).msglen) > 4) {
                ...
                FL_DEBUG("lmp RX monitor_blocking.get_global_paddr_call\n");
                assert(((_binding->rx_vtbl).get_global_paddr_call) != NULL);
                ((_binding->rx_vtbl).get_global_paddr_call)(_binding);
                _binding->rx_msgnum = 0;
                break;
            default:
                (_binding->error_handler)(_binding, FLOUNDER_ERR_INVALID_STATE);
            ...
            break;
        case monitor_blocking_get_global_paddr_response__msgnum:
            switch (_binding->rx_msg_fragment) {
            case 0:
                // check length
                if (((msg.buf).msglen) > 4) {
                ...
                ((_binding->rx_union).get_global_paddr_response).global = ((msg.words)[1]);
                
                FL_DEBUG("lmp RX monitor_blocking.get_global_paddr_response\n");
                assert(((_binding->rx_vtbl).get_global_paddr_response) != NULL);
                ((_binding->rx_vtbl).get_global_paddr_response)(_binding, ((_binding->rx_union).get_global_paddr_response).global);
                _binding->rx_msgnum = 0;
                break;
            default:
                (_binding->error_handler)(_binding, FLOUNDER_ERR_INVALID_STATE);
...
static  errval_t monitor_blocking_get_ipi_cap__rpc(struct monitor_blocking_rpc_client *_rpc, struct capref *cap)
{
    errval_t _err = SYS_ERR_OK;
    assert(!(_rpc->rpc_in_progress));
    assert((_rpc->async_error) == SYS_ERR_OK);
    ...
    // call send function
    _err = ((((_rpc->b)->tx_vtbl).get_ipi_cap_call)(_rpc->b, NOP_CONT));
    if (err_is_fail(_err)) {
        goto out;
    }
    ...
    // grab reply variables out of binding
    struct monitor_blocking_binding *_binding = _rpc->b;
    *cap = (((_binding->rx_union).get_ipi_cap_response).cap);
...
static  errval_t monitor_blocking_forward_kcb_request__rpc(struct monitor_blocking_rpc_client *_rpc, monitor_blocking_coreid_t destination, struct capref kcb, monitor_blocking_errval_t *err)
{
    errval_t _err = SYS_ERR_OK;
    assert(!(_rpc->rpc_in_progress));
    assert((_rpc->async_error) == SYS_ERR_OK);
    ...
    // call send function
    _err = ((((_rpc->b)->tx_vtbl).forward_kcb_request_call)(_rpc->b, NOP_CONT, destination, kcb));
    if (err_is_fail(_err)) {
        goto out;
    }
    ...
    // grab reply variables out of binding
    struct monitor_blocking_binding *_binding = _rpc->b;
    *err = (((_binding->rx_union).forward_kcb_request_response).err);
...
static  errval_t monitor_blocking_forward_kcb_rm_request__rpc(struct monitor_blocking_rpc_client *_rpc, monitor_blocking_coreid_t destination, struct capref kcb, monitor_blocking_errval_t *err)
{
    errval_t _err = SYS_ERR_OK;
    assert(!(_rpc->rpc_in_progress));
    assert((_rpc->async_error) == SYS_ERR_OK);
    ...
    // call send function
    _err = ((((_rpc->b)->tx_vtbl).forward_kcb_rm_request_call)(_rpc->b, NOP_CONT, destination, kcb));
    if (err_is_fail(_err)) {
        goto out;
    }
    ...
    // grab reply variables out of binding
    struct monitor_blocking_binding *_binding = _rpc->b;
    *err = (((_binding->rx_union).forward_kcb_rm_request_response).err);
...
static  errval_t monitor_blocking_get_global_paddr__rpc(struct monitor_blocking_rpc_client *_rpc, monitor_blocking_genpaddr_t *global)
{
    errval_t _err = SYS_ERR_OK;
    assert(!(_rpc->rpc_in_progress));
    assert((_rpc->async_error) == SYS_ERR_OK);
    ...
    // call send function
    _err = ((((_rpc->b)->tx_vtbl).get_global_paddr_call)(_rpc->b, NOP_CONT));
    if (err_is_fail(_err)) {
        goto out;
    }
    ...
    // grab reply variables out of binding
    struct monitor_blocking_binding *_binding = _rpc->b;
    *global = (((_binding->rx_union).get_global_paddr_response).global);
...
static  void monitor_blocking_get_ipi_cap__rpc_rx_handler(struct monitor_blocking_binding *_binding, struct capref cap)
{
    // get RPC client state pointer
    struct monitor_blocking_rpc_client *_rpc = _binding->st;
...
static  void monitor_blocking_forward_kcb_request__rpc_rx_handler(struct monitor_blocking_binding *_binding, monitor_blocking_errval_t err)
{
    // get RPC client state pointer
    struct monitor_blocking_rpc_client *_rpc = _binding->st;
    ...
    // XXX: stash reply parameters in binding object
    // depending on the interconnect driver, they're probably already there
    ((_binding->rx_union).forward_kcb_request_response).err = err;
    
    // notify RPC function, and we're done
    _rpc->reply_present = true;
}

static  void monitor_blocking_forward_kcb_rm_request__rpc_rx_handler(struct monitor_blocking_binding *_binding, monitor_blocking_errval_t err)
{
    // get RPC client state pointer
    struct monitor_blocking_rpc_client *_rpc = _binding->st;
    ...
    // XXX: stash reply parameters in binding object
    // depending on the interconnect driver, they're probably already there
    ((_binding->rx_union).forward_kcb_rm_request_response).err = err;
    
    // notify RPC function, and we're done
    _rpc->reply_present = true;
}

static  void monitor_blocking_get_global_paddr__rpc_rx_handler(struct monitor_blocking_binding *_binding, monitor_blocking_genpaddr_t global)
{
    // get RPC client state pointer
    struct monitor_blocking_rpc_client *_rpc = _binding->st;
    ...
    // XXX: stash reply parameters in binding object
    // depending on the interconnect driver, they're probably already there
    ((_binding->rx_union).get_global_paddr_response).global = global;
...
static  struct monitor_blocking_rpc_vtbl monitor_blocking_rpc_vtbl = {
    ...
    .get_ipi_cap = monitor_blocking_get_ipi_cap__rpc,
    .forward_kcb_request = monitor_blocking_forward_kcb_request__rpc,
    .forward_kcb_rm_request = monitor_blocking_forward_kcb_rm_request__rpc,
    .get_global_paddr = monitor_blocking_get_global_paddr__rpc,
};
 ...
 errval_t monitor_blocking_rpc_client_init(struct monitor_blocking_rpc_client *rpc, struct monitor_blocking_binding *binding)
{
    ...
    (binding->rx_vtbl).get_ipi_cap_response = monitor_blocking_get_ipi_cap__rpc_rx_handler;
    (binding->rx_vtbl).forward_kcb_request_response = monitor_blocking_forward_kcb_request__rpc_rx_handler;
    (binding->rx_vtbl).forward_kcb_rm_request_response = monitor_blocking_forward_kcb_rm_request__rpc_rx_handler;
    (binding->rx_vtbl).get_global_paddr_response = monitor_blocking_get_global_paddr__rpc_rx_handler;
