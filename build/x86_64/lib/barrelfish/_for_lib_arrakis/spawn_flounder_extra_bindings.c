 ...
 */
static  errval_t spawn_spawn_domain__rpc(struct spawn_rpc_client *_rpc, const char *path, const char *argvbuf, size_t argvbytes, const char *envbuf, size_t envbytes, uint8_t flags, spawn_errval_t *err, spawn_domainid_t *domain_id)
{
    ...
    // call send function
    _err = ((((_rpc->b)->tx_vtbl).spawn_domain_call)(_rpc->b, NOP_CONT, path, argvbuf, argvbytes, envbuf, envbytes, flags));
    if (err_is_fail(_err)) {
        goto out;
    }
...
static  errval_t spawn_spawn_domain_with_caps__rpc(struct spawn_rpc_client *_rpc, const char *path, const char *argvbuf, size_t argvbytes, const char *envbuf, size_t envbytes, struct capref inheritcn_cap, struct capref argcn_cap, uint8_t flags, spawn_errval_t *err, spawn_domainid_t *domain_id)
{
    errval_t _err = SYS_ERR_OK;
    assert(!(_rpc->rpc_in_progress));
    assert((_rpc->async_error) == SYS_ERR_OK);
    ...
    // call send function
    _err = ((((_rpc->b)->tx_vtbl).spawn_domain_with_caps_call)(_rpc->b, NOP_CONT, path, argvbuf, argvbytes, envbuf, envbytes, inheritcn_cap, argcn_cap, flags));
    if (err_is_fail(_err)) {
        goto out;
    }
